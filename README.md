...
- `.env` はサーバー起動時に自動で読み込まれます。**`ADMIN_EMAIL` と `ADMIN_PASSWORD` を両方設定すると、2回目以降の起動でもその内容にログイン情報が同期されます**(アカウントが無ければ作成、パスワードが違えば更新)。ログイン情報を固定したい場合やパスワードを忘れた場合は `.env` を書き換えて再起動してください
...
| 画面 | 内容 |
|---|---|
| ホーム | **AI Inbox(至急返信/本日返信推奨/今週中/フォロー推奨の件数)**・今日やること(**最大3件**・優先度スコアリング)・危険案件・KPI・売上サマリー・営業パイプライン・エンジニア稼働状況・AI営業アドバイス |
...
| メール(AI Mail Manager) | 受信メールのAI振り分け(見積依頼/契約相談/質問/クレーム/請求・支払い/開発相談/日程調整/広告・不要メール/雑談/その他 — カテゴリは設定で追加可)・**4段階の緊急度判定(🔴至急/🟠高/🟡中/🟢低)**・要約・**過去のやり取りと案件情報を踏まえたAI返信ドラフト(件名生成・会社テンプレート適用・署名)**・返信漏れ監視・**案件への自動紐付け** |
| 顧客(AI Relationship Manager) | 顧客ごとの最終商談日/最終メール日/最終接触・売上・商談回数・契約件数を自動集計し、**フォロー推奨顧客をAIが提案** |
...
- **危険検知**: 納期遅延・未回収・未請求・商談の滞留・**返信漏れ・クレームの兆候・放置案件・契約未締結**。しきい値は設定画面で変更可能
- **AI営業アドバイス**: 見積提出後の経過日数によるフォロー提案、疎遠顧客のフォロー提案などをホームに表示
...
設定画面からメールアカウントを登録すると、IMAPで受信箱を定期チェック(間隔は設定可)し、新着メールをAIが自動で振り分けます。要返信のメールが一定時間対応されないと、ホームの「危険案件」と「今日やること」に自動で上がります(返信漏れ監視)。返信はAIがドラフトを作成し、**内容を確認・編集してから手動で送信**します(自動送信はしません)。

**Gmailの接続手順**(推奨・確実):

1. Googleアカウントで**2段階認証**を有効にする
2. https://myaccount.google.com/apppasswords で**アプリパスワード**を作成(16文字)
3. 本アプリの設定画面 →「メールアカウント」→ 追加で、プロバイダ `gmail`・メールアドレス・アプリパスワードを登録
4. 「接続テスト」でOKを確認

**Outlookの注意**: プロバイダ `outlook` プリセット(outlook.office365.com)で接続できますが、Microsoftアカウントの種別・組織のポリシーによってはIMAPのパスワード認証が無効化されている場合があります(その場合はOAuth対応が必要で、現バージョンでは未対応)。まずは接続テストでご確認ください。

取得項目は件名・本文・送信者・宛先・CC・添付ファイル名・受信日時・スレッド情報です。Teams / Slack / LINE WORKS / Chatwork の統合は将来対応(データ構造は`source`列で対応済み)。

メールのパスワードはローカルのSQLiteデータベースに保存されます。サーバーを共有環境に置く場合はディスク暗号化等の保護を検討してください。
