...
const tx = db.transaction(() => {
  ...
  const linkMail = db.prepare("UPDATE emails SET project_id = ? WHERE id = ?");
  ...
  const claimMailId = insMail.run("hayashi@sakura-mfg.example.jp", "サクラ製作所 林", "【重要】ECサイトの決済でエラーが発生しています",
    "本日午前より、ECサイトの決済画面でエラーが頻発しています。お客様からの問い合わせも来ており、至急ご確認をお願いします。",
    hoursAgo(3), "クレーム", "至急", "ECサイト決済でエラー発生。至急対応の依頼", 1, "open", null, null).lastInsertRowid;
  linkMail.run(p4, claimMailId);
  ...
  insMail.run("keiri@minato-logi.example.jp", "ミナト物流 経理部", "3月分保守費のご請求について",
    ...
    hoursAgo(8), "請求・支払い", "中", "3月分保守費の請求書送付依頼", 1, "replied", hoursAgo(2),
  ...
  insMail.run("newsletter@saas-magazine.example.com", "SaaSマガジン", "【本日限定】業務効率化ツール特集セミナーのご案内",
    ...
    hoursAgo(12), "広告・不要メール", "低", "セミナー案内のメルマガ", 0, "dismissed", null, null);
});
tx();
